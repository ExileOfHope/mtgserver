...
void CraftingSessionImplementation::createPrototype(int clientCounter, bool createItem) {
	...
	if (manufactureSchematic->isAssembled()
		...
		int xp = 2.0 * (manufactureSchematic->getDraftSchematic()->getXpAmount());

		if (createItem) {

			startCreationTasks(manufactureSchematic->getComplexity() * 0.1, false);

		} else {

			// This is for practicing
			startCreationTasks(manufactureSchematic->getComplexity() * 0.1, true);
			xp = round(xp * 2.00f);
		}
