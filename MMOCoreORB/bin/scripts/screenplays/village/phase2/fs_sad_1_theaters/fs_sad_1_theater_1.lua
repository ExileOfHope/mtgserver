...
function FsSad1Theater1:onObjectsSpawned(pPlayer, spawnedMobileList)
	if (pPlayer == nil) then
		return
	end
...
function FsSad1Theater1:onTheaterCreated(pPlayer)
	if (pPlayer == nil) then
		return
	end
...
function FsSad1Theater1:notifyKilledMobile(pVictim, pAttacker)
	...
	if (numEnemies <= 0) then
		...
		if (pGhost ~= nil) then
			PlayerObject(pGhost):addWaypoint("dathomir", "@quest/quest_journal/fs_quests_sad:return1", "", 5238, -4189, WAYPOINTYELLOW, true, true, WAYPOINTQUESTTASK)
		end
	end
