...
FsSad1Theater4 = GoToTheater:new {
	-- Task properties
	taskName = "FsSad1Theater4",
	...
	theater = {
		...
		{ template = "object/static/structure/corellia/corl_tent_hut_s01.iff", xDiff = -1.393, zDiff = 0.0111, yDiff = 2.875, heading = 0 },
		{ template = "object/static/structure/general/camp_lawn_chair_s01.iff", xDiff = -0.205, zDiff = -0.145, yDiff = -1.388, heading = -145.141 },
		{ template = "object/static/structure/general/campfire_smoldering.iff", xDiff = 2.0294, zDiff = 0.303, yDiff = -3.361, heading = 0 },
...
function FsSad1Theater4:notifyKilledMobile(pVictim, pAttacker)
	...
	if (numEnemies <= 0) then
		QuestManager.completeQuest(pOwner, QuestManager.quests.FS_QUESTS_SAD_TASK4)
		QuestManager.activateQuest(pOwner, QuestManager.quests.FS_QUESTS_SAD_RETURN4)
		deleteData(ownerID .. self.taskName .. ":killableCount")

		local pGhost = CreatureObject(pOwner):getPlayerObject()

		if (pGhost ~= nil) then
			PlayerObject(pGhost):addWaypoint("dathomir", "@quest/quest_journal/fs_quests_sad:return4", "", 5238, -4189, WAYPOINTYELLOW, true, true, WAYPOINTQUESTTASK)
		end
	end
