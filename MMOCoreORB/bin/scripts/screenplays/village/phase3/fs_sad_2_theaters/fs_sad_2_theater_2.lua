...
FsSad2Theater2 = GoToTheater:new {
	-- Task properties
	taskName = "FsSad2Theater2",
	-- GoToTheater properties
	minimumDistance = 800,
	maximumDistance = 2000,
	theater = {
		{ template = "object/static/structure/general/trash_pile_s01.iff", xDiff = -0.9, zDiff = -0.103, yDiff = 0.959, heading = 0 },
	...
	createWaypoint = true,
	waypointDescription = "@quest/quest_journal/fs_quests_sad2:task2",
	activeAreaRadius = 16,
	flattenLayer = true
}

function FsSad2Theater2:onObjectsSpawned(pPlayer, spawnedMobileList)
	if (pPlayer == nil) then
		return
	end
...
function FsSad2Theater2:onTheaterCreated(pPlayer)
	if (pPlayer == nil) then
		return
	end
...
function FsSad2Theater2:notifyKilledMobile(pVictim, pAttacker)
	...
	if (numEnemies <= 0) then
		QuestManager.completeQuest(pOwner, QuestManager.quests.FS_QUESTS_SAD2_TASK2)
		QuestManager.activateQuest(pOwner, QuestManager.quests.FS_QUESTS_SAD2_RETURN2)
		deleteData(ownerID .. self.taskName .. ":killableCount")

		local pGhost = CreatureObject(pOwner):getPlayerObject()

		if (pGhost ~= nil) then
			PlayerObject(pGhost):addWaypoint("dathomir", "@quest/quest_journal/fs_quests_sad2:return2", "", 5238, -4189, WAYPOINTYELLOW, true, true, WAYPOINTQUESTTASK)
		end
	end
